 ...
 * Version:           2.1.2
 * Author:            BrightTALK, Dorian Logan
...
function brighttalk_wp_shortcode($atts, $content=null){
  ...
  $embed = '<script src="https://www.brighttalk.com/clients/js/player-embed/player-embed.js" class="jsBrightTALKEmbed">{"channelId" : %d, "commid" : %d, "height" : "%s", "width" : "100%%", "displayMode" : "%s", "track" : "%s" }</script>';

  $op = sprintf($embed, $brighttalk_shortcode_atts['channelid'], $brighttalk_shortcode_atts['commid'], $brighttalk_shortcode_atts['height'], $brighttalk_shortcode_atts['displaymode'], $track);
...
function brighttalk_wp_time($atts, $content=null){
  ...
  if (isset($_COOKIE["BTSESSION"])) {
    $session = $_COOKIE["BTSESSION"];
    // $tz = brighttalk_getTZFromBTSession($session);
  }

  if (!$tz) {
    // Use WP GeoIP to guess timezone by IP 
    $country = getenv('HTTP_GEOIP_COUNTRY_CODE');
    $region = getenv('HTTP_GEOIP_REGION');
    $tz = brighttalk_region_tz_lookup($country, $region);
  }
